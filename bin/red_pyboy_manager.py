...
class PyBoyManager:
    def __init__(self, env, emulator=None):
        if getattr(env, 'debug', False):
            print('**** PyBoyManager ****')

        self.env = env
        self.pyboy = None
        self.valid_actions = pyboy_init_actions(getattr(self.env, "extra_buttons", False))
        self.action = None
        self.action_history = np.zeros((OBSERVATION_MEMORY_SIZE,), dtype=np.uint8)
        self.move_accepted = True
        
        # Use provided emulator instance if available
        self.emulator = emulator if emulator else None
        if self.emulator:
            self.pyboy = self.emulator.pyboy if hasattr(self.emulator, 'pyboy') else None
